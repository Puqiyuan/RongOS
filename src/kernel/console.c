...
int* ros_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1)
	...
	else if (edx == 5)
		{
			...
			sheet_updown(sht, shtctl->top); // specified to be the same height as the current mouse: mouse will be move on
			reg[7] = (int) sht;
		}
	else if (edx == 6)
		{
	...
	else if (edx == 16)
		{
			...
			((struct TIMER *) reg[7])->flags2 = 1; // automatic cancellation valid
		}
	else if (edx == 17)
		{
