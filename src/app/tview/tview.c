...
int strtol(char *s, char **endp, int base);	/* standistdlib.hj */
...
void HariMain(void)
{
	...
	/* process of command line */
	api_cmdline(s, 30);
	for (p = s; *p > ' '; p++) { }	// read until space
	for (; *p != 0; ) {
		p = skipspace(p);
		if (*p == '-') {
			if (p[1] == 'w') {
	...
	for (;;) {
		...
		if (i == '4') {
			for (;;) {
				...
				if (api_getkey(0) != '4') { /* if not press 4, end of process */
					break;
				}
			}
		}
