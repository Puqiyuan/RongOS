			...
			if ( fifo32_status(&fifo) == 0 )
			...
			else
				{
					...
					if (256 <= i && i <= 511) // keyboard data
									...
									if (key_to == 0)
										{
											...
											cursor_c = -1; // to delete the cursor
											boxfill8(sht_win->buf, sht_win->bxsize, COL8_FFFFFF, cursor_x, 28, cursor_x + 7, 43);
											fifo32_put(&task_cons->fifo, 2); // the cursor is on in console task
										}
									else
										{
											key_to = 0;
											...
											cursor_c = COL8_000000; // place the cursor
						...
						{
							if (mouse_decode(&mdec, i - 512) != 0)// since the 3 bytes of data are prepared, it is displayed
								{
...
void make_wtitle8(unsigned char *buf, int xsize, char *title, char act)
{
	...
	if (act != 0)
		{
			tc = COL8_FFFFFF; // white
			tbc = COL8_000084; // dark blue
		}
	else
		{
			tc = COL8_C6C6C6; // light gray
			tbc = COL8_848484; // dark  gray
		}
			...
			for (x = 0; x < 16; x++)
				{
					...
					if (c == '@')
						...
						c = COL8_848484; // dark gray
					else if (c == 'Q')
						c = COL8_C6C6C6; // light gray
					else
						c = COL8_FFFFFF;
