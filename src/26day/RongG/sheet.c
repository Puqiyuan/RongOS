			...
			if (ctl -> sheets0[i].flags == 0)
				{
					...
					sht -> height = -1; // hidden
					sht -> task = 0; // do not use automatic close function
					return sht;
				}
		}
...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl -> top; h++)
		{
			sht = ctl -> sheets[h];
			sid = sht - ctl -> sheets0; // subtract the sheets0 address and use it as a number
			...
			if (sht->col_inv == -1)
				{
					if ((sht->vx0 & 3) == 0 && (bx0 & 3) == 0 && (bx1 & 3) == 0)
						{
							// high-speed version dedicated to transparent none (4 byte type)
							bx1 = (bx1 - bx0) / 4; // how many times move
				...
				{
					// general edition with transparent color
					for (by = by0; by < by1; by++)
						{
							vy = sht -> vy0 + by;
							for (bx = bx0; bx < bx1; bx++)
	...
	for (h = h0; h <= h1; h++)
		{
			...
			if ((sht->vx0 & 3) == 0)
				{
					// 4 byte type
					i		= (bx0 + 3) / 4; // bx0 divided by 4 (rounded up)
					i1		= bx1 / 4; // bx1 divided by 4 (rounded down)
					i1 = i1 - i;
					sid4	= sid | sid << 8 | sid << 16 | sid << 24;
					for (by = by0; by < by1; by++)
						{
							vy = sht->vy0 + by;
							for (bx = bx0; bx < bx1 && (bx & 3) != 0; bx++) // one by one for the previous fraction
								{
									vx = sht->vx0 + bx;
							...
							for (i = 0; i < i1; i++) // a multiple of 4
								{
									if (p[i] == sid4)
										{
											q[i] = r[i];
							...
							for (bx += i1 * 4; bx < bx1; bx++) // one by one for back fraction
								{
									vx = sht->vx0 + bx;
									if (map[vy * ctl->xsize + vx] == sid)
										{
