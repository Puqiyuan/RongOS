...
int mouse_decode(struct MOUSE_DEC *mdec, unsigned char dat)
{
	...
	if (mdec -> phase == 3)// waiting for the third byte of the mouse
		{
			mdec -> buf[2] = dat;
			mdec -> phase = 1;
			...
			mdec -> y = - mdec -> y; // in the mouse, the sign in the y direction is the opposite of the screen.
			
			return 1;
		}

	return -1; // there should not be anything to come here.
}
