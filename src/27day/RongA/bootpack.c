			...
			if ( fifo32_status(&fifo) == 0 )
			...
			else
				{
					...
					if (key_win != 0 && key_win->flags == 0) // window closed
						{
							if (shtctl->top == 1) // only mouse and background exist
								{
									key_win = 0;
								}
							else
					...
					if (256 <= i && i <= 511) // keyboard data
						{
							...
							if (s[0] != 0 && key_win != 0) // ordinary character
								{
									...
									if (task != 0 && task->tss.ss0 != 0)
										{
											...
											io_sti();
											task_run(task, -1, 0); // I will wake up when I am asleep so that I can finish it exactly
										}
								}
					...
					else if (768 <= i && i <= 1023) // console termination processing
						{
