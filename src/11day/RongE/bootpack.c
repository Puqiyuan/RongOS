...
void HariMain(void)
{
	...
	putfonts8_asc(buf_win, 160, 24, 44, COL8_000000, "  Rongbote-OS!");
							...
							if (mouse_decode(&mdec, i) != 0)// since the 3 bytes of data are prepared, it is displayed
								{
									...
									if (my > binfo -> scrny - 1)
										my = binfo -> scrny - 1;
									sprintf(s, "(%3d, %3d)", mx, my);
									boxfill8(buf_back, binfo -> scrnx, COL8_008484, 0, 0, 79, 15); // to delete coordinates.
									putfonts8_asc(buf_back, binfo -> scrnx, 0, 0, COL8_FFFFFF, s); // write coordinates.
