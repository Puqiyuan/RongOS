...
this table is an member \texttt{struct FREEINFO} in \texttt{struct MEMMAN}.
...
\begin{itemize}
\item Check the memory capacity and confirm that the memory is intact. \texttt{start} is the start
  address of the memory check and \texttt{end} is the end address of the memory
  check. This function returns the maximum value of available addresses. Memory is
  available between this value and the \texttt{start} value.
\end{itemize}
...
\item Report the sum of all empty space. It will look for each \emph{entry} in the
  \texttt{man} to add up the free space in it.
...
\item Allocate memory to the application, where \texttt{size} is the space requested by the
  application. Success returns the available starting address, otherwise 0. \texttt{man} records
...
\begin{itemize}
\item Basically it is similar to the \texttt{memman\_alloc} function, but memory is allocated
  in 4k memory units and the starting address of the allocated memory is returned if
  successful. \texttt{size} is the size of requested space.
\end{itemize}

\csingle|int memman_free_4k(struct MEMMAN *man, unsigned int addr, unsigned int size);|
\begin{itemize}
\item Basically it is similar to the \texttt{memman\_free}, but memory space is freed in
  units of 4k, \texttt{addr}is the starting point variable, and \texttt{size} is the
  release size.
\end{itemize}
...
\item holds information about task status segments, which are based on CPU
  specifications\cite[Sec.6.2.1]{intel_3a}. \fxerror{more about these registers}
\end{itemize}
...
\end{codeblock}
\begin{itemize}
\item is used to control all tasks in the system.\fxerror{not clear. the memory manage structure}
\end{itemize}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=.7\textwidth]{flowchartp}
...
function. \texttt{size} refers to the space requested by the application but \texttt{SIZE}
is the free space of one \emph{entry}. \texttt{free} is the number of memory available
memory.
...
\item Add \texttt{size} to start address of \emph{entry} found in 1. Subtract 
  \texttt{size} from free space \texttt{SIZE} in \emph{entry} found in 1. Jump to 3.
\item Determine if the remaining free space \texttt{SIZE} in the entry is 0. If so, go to
  4. Otherwise 5.
\item Subtract the number of memory available entries \texttt{free} by 1.
...
\item Neither merge with the previous \emph{entry} nor with the latter \emph{entry}. For
  ...
  \begin{figure}[!ht]
    ...
    \label{fig:no-merge}
  \end{figure}
  As shown in the figure, the starting address of \texttt{new x} 803 is not adjacent to
...
\item Can be merged with the previous entry. As the figure ~\ref{fig:former-merge} shows.
  ...
  \end{figure}
  As shown in the figure, the starting address of \texttt{new x} 670 is adjacent to the
...
\item Can be merged with the latter entry. As the figure ~\ref{fig:latter-merge} shows.
  ...
  \end{figure}
  The termination address 999 of \texttt{new x} is adjacent to the starting address 1000
  of the next entry, but its starting address of \texttt{new x} 803 is not adjacent to the
  previous termination address 669.
  
\end{enumerate}
The memory release process can be described as follows.
\begin{enumerate}
\item Find the insertion position \emph{i}. The so-called insertion position is an \texttt{entry}
  number that satisfies certain conditions. This condition is that the \texttt{size} of this
  \texttt{entry} is larger than the size requested by the application.
\item If $i > 0$, merge if the newly inserted \emph{entry} can be merged with the previous \texttt{entry},
...
\item The program proceeds here so that the newly inserted \texttt{entry} must not be
  merged with the previous \texttt{entry}. Now check if the newly inserted \texttt{entry}
  can be merged with the latter \texttt{entries}. If so, merges it and return 0. Otherwise
  jump to 4.
\item The program proceeds here to represent that the newly inserted \texttt{entry} cannot be
  ...
  make room for new items. Then insert this new \texttt{entry}. Otherwise jmup to 5.
...
hierarchically. From \emph{level 0} to \emph{level 10}, there are a total of ten levels in
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=.7\textwidth]{task-run}
...
\end{enumerate}
Specific code implementation, see ~\ref{sec:task-scheduling} and ~\ref{sec:task-running}.
