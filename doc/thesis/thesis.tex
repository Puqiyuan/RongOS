...
\begin{description}
\item[\mintinline{c}{void memman_init(struct MEMMAN* man);}]\ \par The memory space is
  initialized and the \texttt{man} variable is used to record the memory space usage and
  free information.
\item[\mintinline{c}{unsigned int memman_total(struct MEMMAN* man);}]\ \par Report the sum
  of all empty space. Return the size of free memory.
\item[\mintinline{c}{unsigned int memman_alloc(struct MEMMAN *man, unsigned int
    size);}]\ \par
...
\item[\mintinline{c}{int memman_free(struct MEMMAN *man, unsigned int addr, unsigned int
    size);}]\ \par
...
\texttt{struct TASKLEVEL} (Code~\ref{src:TASKLEVEL}) is used to record the status of each task in a
layer.
...
\subparagraph{Driver Functions}

\begin{description}
  
\item[\mintinline{c}{void set_palette(int start, int end, unsigned char *rgb)}] initialize the palette.
\item[\mintinline{c}{void init_pic(void);}] initialize the PIC.
\item[\mintinline{c}{void init_keyboard(struct FIFO32 *fifo, int data0);}] initialze the
  keyboard.
\item[\mintinline{c}{void enable_mouse(struct FIFO32 *fifo, int data0, struct  MOUSE_DEC
    *mdec);}] activate mouse
...
\label{sec:fat}
The file system defines how data is stored and accessed. The file system divides the
entire storage space according to a certain form. This facilitates the storage of files
while increasing the utilization of storage media. The small blocks thus divided are
called sectors. The FAT records which sectors all files are stored on. 
  
\subparagraph{FAT Data Structure}
