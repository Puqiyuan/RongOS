...
\begin{tikzpicture}[%
  ...
  %\node (nfs)  [draw,below=3ex of vfs.south, scale=.8, xshift=-1cm] {FAT};%
  ...
  %\draw [arrow] (nfs) -- ($(drv.north) + (-1.55em, 0)$);
  ...
  \node (lib) [draw,above=of hw.north east,anchor=south east,minimum width=4cm,
  yshift=2.5cm]%
  {Libraries};%

  \draw [dashed] ($(lib.south west) + (-6cm,-5mm)$)--%
  node [above,xshift=-4.8cm,scale=.6] {User level}%
  node [below,xshift=-4.65cm,scale=.6]{Kernel level}%
  ($(lib.south east) + (0.5cm,-5mm)$);%

  \draw [dashed] ($(hw.north west) + (-0.25cm,5mm)$)--%
  node [above,xshift=-4.5cm,scale=.6] {Kernel level}%
  node [below,xshift=-4.5cm,scale=.6]{Hardware level}%
  ($(hw.north east) + (0.25cm,5mm)$);%

  \node (user) [above=of lib.north west,anchor=east] {User programs};%
  \node (kernel) [rectangle, rounded corners, minimum width=8cm, minimum
height=1.8cm, align=center, draw, below=of lib, xshift=-3cm, yshift=0.1cm, label={[anchor=center
  , inner sep=1.5pt]center:$Kernel$}] {};
...
\draw [arrow] (lib.south)--node [rotate=0,above,scale=.8, xshift=-0.5cm] {trap} (lib |- kernel.north);%
